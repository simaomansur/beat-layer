...
from src.app.forms import (
    SignUpForm, BeatForm, ForgotPasswordForm,
    ResetPasswordForm, HomeForm, MyProfileForm
)
from flask import (
    render_template, redirect, url_for, flash,
    send_from_directory, jsonify, request
)
...
def beats():
    ...
    if genre:
        beat_tuples = Beat.query.filter_by(genre=genre).join(
            User, User.id == Beat.artist).add_columns(User.profile_pic).all()
    else:
        beat_tuples = (
            Beat.query
            .join(User, User.id == Beat.artist)
            .add_columns(User.profile_pic)
            .all()
        )
    beats_with_pics = [
        {'beat': beat, 'profile_pic': profile_pic}
        for beat, profile_pic in beat_tuples
    ]
...
def beats_new():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            db.session.rollback()
            flash(
                'An error occurred while creating your beat. '
                'Please try again.'
                )
            print(e)
    else:
...
def my_profile():
    ...
    if form.validate_on_submit():
        if form.profile_pic.data:
            ...
            current_user.profile_pic = os.path.join('pictures',
                                                    'profile_pictures',
                                                    filename
                                                    )
...
def send_password_reset_email(user_email):
    ...
    email = Message("Password Reset Requested",
    ...
    email.body = (
        ...
        f'If you did not make this request then simply ignore this email and '
...
def like_song(beat_id):
    ...
    return jsonify(
        {"success": True,
         "likes": Like.query.filter_by(beat_id=beat_id).count()
         })
